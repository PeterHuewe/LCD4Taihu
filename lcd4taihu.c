    ...
    else
    {
        if(!copy_from_user(ks_buf,buf,count))
        {
            for (i=0; i< count;i++)
            {
                ...
                udelay(2000);
                g_addr++;
                if(g_addr & 0x10) //end of lineclass_ reached - change to other line
                {
        ...
        else
        {
            ...
            printk("Copy from user failed! Value %s anzahl %d\n",ks_buf,count);
        }
        kfree(ks_buf);
    }
    return count;
...
static ssize_t store_hex_cmd (struct class_device *dev, const char *buffer,size_t size)
{
...
static ssize_t store_hex_data (struct class_device *dev,const char *buffer,size_t size)
{
    char new = simple_strtol (buffer,NULL,16);
    iowrite8(new, (void*)data_mmap);
    return 4;
}
static ssize_t store_cmd (struct class_device *dev, const char *buffer,size_t size)
{
    printk(KERN_ERR "%d\n", buffer[0]);
...
static ssize_t store_data (struct class_device *dev, const char *buffer,size_t size)
{
...
static ssize_t get_backlight (struct class_device *dev,char *buffer)
{
    ssize_t len =0 ;
...
static ssize_t set_backlight (struct class_device *dev,  const char *buffer,size_t size)
{
    char new;
    char backlight = ioread8((void *)bckl_mmap);
...
CLASS_DEVICE_ATTR(hex_cmd,S_IWUSR,NULL,store_hex_cmd);
CLASS_DEVICE_ATTR(hex_data,S_IWUSR,NULL,store_hex_data);
CLASS_DEVICE_ATTR(cmd,S_IWUSR,NULL,store_cmd);
CLASS_DEVICE_ATTR(data,S_IWUSR,NULL,store_data);
CLASS_DEVICE_ATTR(backlight,S_IRUGO|S_IWUSR,get_backlight,set_backlight);
...
static int __init taihu_lcd_init(void) {
    ...
    status=class_device_create_file(taihu_miscdev.class,&class_device_attr_hex_cmd);
    status=class_device_create_file(taihu_miscdev.class,&class_device_attr_hex_data);
    status=class_device_create_file(taihu_miscdev.class,&class_device_attr_data);
    status=class_device_create_file(taihu_miscdev.class,&class_device_attr_cmd);
    status=class_device_create_file(taihu_miscdev.class,&class_device_attr_backlight);
    return 0;
...
static void __exit taihu_lcd_cleanup(void){
    class_device_remove_file(taihu_miscdev.class,&class_device_attr_backlight);
    class_device_remove_file(taihu_miscdev.class,&class_device_attr_cmd);
    class_device_remove_file(taihu_miscdev.class,&class_device_attr_data);
    class_device_remove_file(taihu_miscdev.class,&class_device_attr_hex_data);
    class_device_remove_file(taihu_miscdev.class,&class_device_attr_hex_cmd);
